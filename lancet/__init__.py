...
class StaticArgs(BaseArgs):
    ...
    def extract_log_specification(log_path, dict_type=dict):
        """
        Parses the log file generated by a launcher, returning a dictionary
        indexing the specifications by tid. Note that keys are converted from
        unicode to string to allow use as kwargs.

        Item ordering can be maintained by setting dict_type to an appropriate
        constructor, typically of an ordered dictionary type.
        """
